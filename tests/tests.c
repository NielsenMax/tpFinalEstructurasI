...
static void test_lista() {
  printf("TESTEANDO FUNCIONES DE lista.h\n");
  {
    ...
    Lista lista =
        lista_crear(8, copiar_puntero_entero, destruir_puntero_entero);
    int numeros[] = { 23, 56, 78, 21, 2, 35, 49, 64 };
    for (int i = 0; i < 8; i++)
    ...
    //test de insertar mas que la capacidad
  ...
  }
  {
    ...
    //El actual tiene que ser NULL porque terminamos la lista.
    assert(!lista->act);
    //El primero no deberia cambiar.
    assert(*(int *) lista->prm->dato == numeros[0]);
    assert(lista_largo(lista) == 8);
    printf("TESTEANDO FUNCION lista_siguiente\n");
    for (int i = 0; i < 8; i++)
      assert(*(int *) lista_siguiente(lista) == numeros[i]);
    //Actual tendria que apuntar al ultimo
    assert(*(int *) lista->act->dato == numeros[7]);
    assert(lista_largo(lista) == 8);
    lista_destruir(lista);
...
static void test_array() {
  printf("TESTEANDO FUNCIONES DE array.h\n");
  {
    ...
    Array array =
        array_crear(8, copiar_puntero_entero, destruir_puntero_entero);
    int numeros[] = { 23, 56, 78, 21, 2, 35, 49, 64 };
    for (int i = 0; i < 8; i++)
  ...
  }
  {
    ...
    Array array =
        array_crear(12, copiar_puntero_entero, destruir_puntero_entero);
...
static void test_contacto() {
  ...
  }
  {
    ...
    assert(contacto_comparar_edad(a, b) == 0
           && contacto_comparar_telefono(a, b) == 0);
    contacto_destruir(a);
    //contacto_destruir(b);
  ...
  }
  {
    ...
    assert(!contacto_comparar_edad(b, a)
           && !contacto_comparar_telefono(b, a));
    contacto_destruir(a);
    //contacto_destruir(b);
